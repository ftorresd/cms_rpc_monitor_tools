...
def process_currents_by_dpid(channel, dpid, HV_point, epsilon=70):
    ...
    for (
        ...
        if ACTUAL_IMON != None and is_stable == False:
    ...
    result = *crm.get_channel_meta(channel), dpid, average(currents, last_imon)
    return result


def main():
    results = []
    for channel in tqdm([*crm.channels]):
        results.append(process_currents_by_dpid(channel, crm.channels[channel], 8000))

    with open("outputs/outputs.csv", "w") as out:
